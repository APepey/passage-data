...
sites_gdf = gpd.GeoDataFrame(
    dataset,
    geometry=gpd.points_from_xy(dataset["Longitude"].astype(float), dataset["Latitude"].astype(float)),
    crs="EPSG:4326",
)

gps_gdf = gpd.GeoDataFrame(
    alltracks,
    geometry=gpd.points_from_xy(alltracks["x__geo_"].astype(float), alltracks["y__geo_"].astype(float)),
    crs="EPSG:4326",
)

# %% Buffer analysis with source breakdown
buffer_distances_m = [500, 1000, 2000]
source_types = ["2018", "2019", "2018+2019", "F1", "F2", "F1+F2"]
results = []
...
for idx, site in sites_gdf.iterrows():
    ...
    for buffer_m in buffer_distances_m:
        ...
        for src in source_types:
            ...
            if src == "2018+2019":
                src_count = len(points_in_buffer[points_in_buffer["source"].isin(["2018", "2019"])])

            elif src == "F1+F2":
                src_count = len(points_in_buffer[points_in_buffer["source"].isin(["F1", "F2"])])
