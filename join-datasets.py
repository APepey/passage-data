...
# %%
cols = [
    'Count2018', 'CountPos2019', 'Count2019',
    'CountSubCohortF1', 'CountSubCohortF2',
    'CountSubCohort', 'CountTotal'
]

result = (
    full_dataset.groupby('Passage')[cols]
      .agg(['min', 'max', 'mean'])
)
result.to_csv('outputs/stats.csv')
# %% only HDN
result_hdn = (
    full_dataset[full_dataset['TrapType'] == 'hdn']
...
result_hdn_novillage = (
    full_dataset[
        (full_dataset['TrapType'] == 'hdn') &
        (full_dataset['LU'] != 'village')
    ]
    .groupby('Passage')[cols]
    .agg(['min', 'max', 'mean'])
)
